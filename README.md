...
The folder [`examples/`](examples/generated-before-after.md) contains real samples generated by the same model, with and without the skill loaded. Both runs use `pi` with GitHub Copilot at `claude-sonnet-5`. Neither has access to CLAUDE.md, auto-discovered skills, or project context. The only difference is whether the skill is in the system prompt.

The script that produced the samples is at [`evals/run.sh`](evals/run.sh). Re-run it any time with `./evals/run.sh` to regenerate against a newer model.

One effect the skill produces reliably: for a prompt with missing reader information, the model asks for it before drafting rather than making assumptions. Prompt used:
...
For an RFC prompt with more content to work with, the baseline produces a competent generic RFC; the skill produces a structured document with an "Are you the right reader?" gate, per-team action items with time estimates, an alternatives-considered table, and explicit success criteria. Full outputs in [`examples/generated-before-after.md`](examples/generated-before-after.md).
